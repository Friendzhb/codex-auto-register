...
# 邮箱配置（优先使用 MoeMail，未配置时回退到旧 Cloudflare Worker）
...
def create_temp_email(session):
    ...
    if use_moemail():
        try:
            ...
            name = "".join(random.choice(chars) for _ in range(random.randint(8, 13)))
            res = session.post(
                f"{MOEMAIL_API_URL}/api/emails/generate",
                json={
                    "name": name,
            ...
            if res.status_code == 200:
                ...
                mailbox_token = data.get("id") or data.get("emailId")
                if email and mailbox_token:
                    print(f"  ✅ 邮箱: {email}")
                    return email, mailbox_token
                print(f"  ❌ 返回格式异常: {data}")
            else:
                print(f"  ❌ 创建失败: {res.status_code} {res.text[:200]}")
        except Exception as e:
            print(f"  ❌ 异常: {e}")
        return None, None
...
def fetch_emails(session, email, mailbox_token):
    """获取邮箱中的邮件"""
    if use_moemail():
        try:
            res = session.get(
            ...
            for item in messages:
                ...
                source = item.get("source") or item.get("from") or "未知"
                if not raw:
                    detail = session.get(
                        f"{MOEMAIL_API_URL}/api/emails/{mailbox_token}/{msg_id}",
                        headers={"X-API-Key": MOEMAIL_API_KEY},
                        verify=False, timeout=30,
                    )
                    if detail.status_code == 200:
                        msg_data = detail.json()
                        raw = msg_data.get("html") or msg_data.get("text") or ""
                        subject = msg_data.get("subject") or subject
                        source = msg_data.get("source") or msg_data.get("from") or source
                normalized.append({
                    "id": msg_id,
                    "raw": raw,
                    "subject": subject,
...
def wait_for_verification_code(session, email, mailbox_token, timeout=120):
    """等待验证邮件并提取验证码"""
    print(f"  ⏳ 等待{get_mailbox_label()}验证码 (最大 {timeout}s)...")
    # 记录旧邮件 ID
    old_ids = set()
    old = fetch_emails(session, email, mailbox_token)
    if old:
    ...
    while time.time() - start < timeout:
        poll_count += 1
        emails = fetch_emails(session, email, mailbox_token)
        if emails:
            if poll_count <= 3:
...
class ProtocolRegistrar:
    ...
    def register(self, email, mailbox_token, password):
        """
        ...
        try:
            ...
            code = wait_for_verification_code(mail_session, email, mailbox_token)
            if not code:
                print("❌ 未收到验证码")
                return False, email, password
...
def perform_codex_oauth_login_http(email, password, registrar_session=None, mailbox_token=None, cf_token=None):
    """
    ...
    if page_type == "email_otp_verification" or "email-verification" in continue_url:
        ...
        # 记录初始邮件数量（注册阶段的）
        initial_emails = fetch_emails(mail_session, email, mailbox_token)
        ...
        while time.time() - start_time < 120:
            all_emails = fetch_emails(mail_session, email, mailbox_token)
            if not all_emails:
                time.sleep(2)
                continue
...
def run_batch():
    ...
    mailbox_domain = MOEMAIL_DOMAIN if use_moemail() else CF_EMAIL_DOMAIN
    print(f"\n🚀 协议注册机 v5 — {TOTAL_ACCOUNTS} 个账号 | 并发 {workers} | 邮箱服务 {get_mailbox_label()} | 域名 {mailbox_domain}")
...
def _save_config():
    """将当前全局配置写回 config.json（仅更新可交互字段）。"""
    try:
        existing = {}
...
def _test_flaresolverr(url: str) -> tuple:
    ...
    """
    try:
        resp = create_session().get(url.rstrip("/") + "/health", timeout=8)
        if resp.status_code == 200:
            return True, ""
...
def _interactive_setup():
    ...
    global FLARESOLVERR_URL, PROXY, TOTAL_ACCOUNTS, CONCURRENT_WORKERS
    ...
    while True:
        ...
        if not key or key.upper() == PLACEHOLDER_API_KEY.upper():
    ...
    # ── 4. 代理 ────────────────────────────────────────────────
    PROXY = _prompt("代理地址（如 http://127.0.0.1:7890，留空不使用）",
                    default=PROXY) or ""

    # ── 5. 验证 MoeMail（带重试）──────────────────────────────
    print("\n🔍 正在验证 MoeMail API 连通性...")
    while True:
        ok, err = _test_moemail(MOEMAIL_API_URL, MOEMAIL_API_KEY, MOEMAIL_DOMAIN, proxy=PROXY)
        if ok:
        ...
        if url:
            MOEMAIL_API_URL = url.rstrip("/")
        key = _prompt("MoeMail API Key", default=MOEMAIL_API_KEY, secret=True)
        if key and key.upper() != PLACEHOLDER_API_KEY.upper():
            MOEMAIL_API_KEY = key
        domain = _prompt("MoeMail 邮箱域名", default=MOEMAIL_DOMAIN)
        if domain:
            MOEMAIL_DOMAIN = domain
    ...
    # ── 7. 注册数量 / 并发数 ───────────────────────────────────
    count_raw = _prompt("注册账号数量", default=str(TOTAL_ACCOUNTS))
    try:
        TOTAL_ACCOUNTS = max(1, int(count_raw))
    except ValueError:
        print(f"  ⚠️ 无效数字，保持 {TOTAL_ACCOUNTS}")

    workers_raw = _prompt("并发数", default=str(CONCURRENT_WORKERS))
    try:
        CONCURRENT_WORKERS = max(1, int(workers_raw))
    except ValueError:
        print(f"  ⚠️ 无效数字，保持 {CONCURRENT_WORKERS}")
